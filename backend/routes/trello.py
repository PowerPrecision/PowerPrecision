...
# ====================================================================
# WEBHOOK TRELLO (BIDIRECIONAL)
# ====================================================================
# Recebe notificações do Trello quando cartões são movidos/actualizados
# ====================================================================

class TrelloWebhookPayload(BaseModel):
    """Payload recebido do webhook do Trello."""
...
async def trello_webhook_verification():
    """
    Verificação HEAD do Trello para setup do webhook.
    O Trello faz uma chamada HEAD antes de aceitar o webhook.
    """
    return {"status": "ok"}
...
async def trello_webhook(
    ...
    """
    Recebe notificações do Trello via webhook.
    
    Eventos processados:
    - updateCard: Cartão movido entre listas → atualiza status do processo
    - addMemberToCard: Membro adicionado → atualiza consultor/mediador
    - removeMemberFromCard: Membro removido
    
    O ID do processo é extraído da descrição do cartão.
    """
    try:
        payload = await request.json()
    except Exception:
        # Se não conseguir parsear JSON, apenas retornar OK (verificação do Trello)
        return {"status": "ok"}
    ...
    if not action_type:
        return {"status": "ok", "message": "No action type"}
    
    logger.info(f"Trello webhook recebido: {action_type}")
    
    # Processar em background para responder rápido ao Trello
    background_tasks.add_task(process_trello_webhook, action)
    
    return {"status": "ok", "action_type": action_type}


async def process_trello_webhook(action: dict):
    """
    Processa o webhook do Trello em background.
    """
    action_type = action.get("type")
    ...
    if not card_id:
        logger.warning("Webhook sem card_id")
        return
    
    logger.info(f"Processando webhook: {action_type} para cartão {card_id}")
    
    try:
        # Encontrar processo pelo trello_card_id
        ...
        if not process:
            # Tentar extrair process_id da descrição do cartão
            card_desc = card_data.get("desc", "")
            process_id = extract_process_id_from_description(card_desc)
        ...
        if not process:
            logger.warning(f"Processo não encontrado para cartão {card_id}")
            return
        
        # Processar diferentes tipos de acções
        if action_type == "updateCard":
            await handle_card_update(action, process)
        elif action_type == "addMemberToCard":
            await handle_member_added(action, process)
        elif action_type == "removeMemberFromCard":
            await handle_member_removed(action, process)
        else:
            logger.debug(f"Acção {action_type} ignorada")
            
    except Exception as e:
        logger.error(f"Erro ao processar webhook Trello: {e}")


def extract_process_id_from_description(description: str) -> Optional[str]:
    """
    Extrai o ID do processo da descrição do cartão Trello.
...
async def handle_card_update(action: dict, process: dict):
    """
    Processa movimento de cartão entre listas.
    Atualiza o status do processo correspondente.
    """
    data = action.get("data", {})
    
    # Verificar se foi movimento entre listas
    list_before = data.get("listBefore", {})
    list_after = data.get("listAfter", {})
    
    if not list_after.get("name"):
        logger.debug("Não é um movimento de lista")
        return
    
    new_list_name = list_after.get("name")
    old_list_name = list_before.get("name", "")
    
    # Converter nome da lista para status do sistema
    ...
    if process.get("status") == new_status:
        logger.debug(f"Status já é {new_status}, ignorando")
        return
...
async def handle_member_added(action: dict, process: dict):
    """
    Processa adição de membro ao cartão.
    Tenta atribuir como consultor ou mediador.
    """
    ...
    if user.get("role") in ["consultor", "admin", "ceo"]:
...
async def handle_member_removed(action: dict, process: dict):
    """
    Processa remoção de membro do cartão.
    """
    member = action.get("member", {})
    trello_username = member.get("username", "").lower()
