...
@router.post("/search/idealista")
async def search_idealista_properties(
    location: str = Query("Lisboa", description="Localização (cidade)"),
    operation: str = Query("sale", description="Tipo de operação: sale ou rent"),
    min_price: Optional[float] = Query(None, description="Preço mínimo"),
    max_price: Optional[float] = Query(None, description="Preço máximo"),
    min_rooms: Optional[int] = Query(None, description="Quartos mínimos"),
    max_rooms: Optional[int] = Query(None, description="Quartos máximos"),
    max_results: int = Query(20, description="Máximo de resultados"),
    user: dict = Depends(get_current_user)
):
    """
    Pesquisar imóveis no Idealista via API oficial.
    
    Requer IDEALISTA_API_KEY e IDEALISTA_API_SECRET configurados.
    """
    from services.idealista_api import search_idealista, idealista_api
    
    if not idealista_api.is_configured:
        raise HTTPException(
            status_code=503,
            detail="API do Idealista não configurada. Configure IDEALISTA_API_KEY e IDEALISTA_API_SECRET."
        )
    
    logger.info(f"Pesquisa Idealista: {location}, {operation}, {min_price}-{max_price}, T{min_rooms}-T{max_rooms}")
    
    result = await search_idealista(
        location=location,
        operation=operation,
        min_price=min_price,
        max_price=max_price,
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=500,
            detail=result.get("error", "Erro na pesquisa Idealista")
        )
    
    return result


@router.get("/search/idealista/status")
async def check_idealista_status(
    user: dict = Depends(get_current_user)
):
    """Verificar se a API do Idealista está configurada."""
    from services.idealista_api import idealista_api
    
    return {
        "configured": idealista_api.is_configured,
        "message": "API Idealista configurada" if idealista_api.is_configured else "Configure IDEALISTA_API_KEY e IDEALISTA_API_SECRET"
    }
